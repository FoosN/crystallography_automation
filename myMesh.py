...
def autoMesh(
    ...
    for omega in [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]:
        ...
        #        print numpyImage
        filteredImage = filterDifferenceImage(difference_image) #This is increasing contrast pushing yellow to max as well as the blue.
        if debug:
            plot_img(
                filteredImage,
                plot_path=os.path.join(
        ...
        (list_index, list_upper, list_lower) = loopExam(filteredImage) # analyse one image (the filtered one)
        if debug:
            ...
            pyplot.close()
        dict_loop["%d" % omega] = (list_index, list_upper, list_lower) # stored in a dict the loopExam results (analyse resulting from filtered image)
    are_the_same_image = checkForCorrelatedImages(dict_loop) # check that the image in dict and the one filtered are the same
...
""" def subtractBackground(image, background_image): # substract bckground from the image and return an array
...
def findOptimalMesh(
    ...
    for omega in [0, 30, 60, 90, 120, 150]:
        ...
        if mesh_xmax is None:
            mesh_xmax = numpy.max(array_index1)  # definition of mesh_xmin
    ...
    mesh_xmin = mesh_xmax - loop_max_width   # definition of mesh_xmin 
    ...
    for omega in [0, 30, 60, 90, 120, 150]:
        ...
        #NF    pylab.plot(array_upper2, "+", color="blue")
            pylab.plot(array_lower1, "+", color="red")
        #NF    pylab.plot(array_lower2, "+", color="blue")
            pylab.savefig(
                os.path.join(
                    auto_mesh_working_dir,
...
""" def plotImage(image):
    ...
    pyplot.show()  #plot image (greenish yellow) """
